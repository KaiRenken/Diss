...
\]
If $n$ is divided by $k+2$, then we have:
\[
...
The $k$-th Cheeger constant of a simplicial complex $X$ equals $0$ iff the $k$-th homology group $H_k(X)$ is non-trivial, as follows:
\pause
...
\frame{\frametitle{Sketch of a proof}



}

\section{Cut-minimal graphs and Cheeger graphs of a simplex}

\frame{\frametitle{Cut-minimal graphs and Cheeger graphs of a simplex}
\pause
\begin{Definition}
...
There is a one-to-one correspondence between the cut-minimal graphs on $n$ vertices and the $1$-cosystoles in $C^1(\Delta^{[n]})$.
\pause
\begin{Definition}
A cut-minimal graph which corresponds to a Cheeger cosystole is called a \textbf{Cheeger graph}.
\end{Definition}
}

\frame{\frametitle{Maximal cut-minimal graphs}
