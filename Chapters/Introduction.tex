...
\\
Let \(X\) be a simplicial complex. For a cochain \(\varphi\in C^k(X)\) we define the \textbf{norm}\index{Norm} of \(\varphi\) as \(\|\varphi\|\coloneqq |\supp(\varphi)|\). Let now \(\varphi\in C^k(X)\), such that \(\|\delta^{k-1}(\phi)+\varphi\|\geq\|\varphi\|\) holds for every \(\phi\in C^{k-1}(X)\), then we call \(\varphi\) a \(k\)-\textbf{cosystole}\index{Cosystole}. For general cochains \(\varphi\in C^k(X)\) we define the \textbf{cosystolic norm}\index{Cosystolic norm} of \(\varphi\) by
\[
...
Furthermore, any \(c\in \varphi+\im(\delta^{k-1})\) satisfying \(\|c\|=\|\varphi\|_{csy}\) is called a \textbf{cosystolic form}\index{Cosystolic form} of \(\varphi\).\\
\\
For a cochain \(\varphi\in C^k(X)\setminus\im(\delta^{k-1})\) the quotient
\[
\|\varphi\|_{exp}\coloneqq \frac{\|\delta^k(\varphi)\|}{\|\varphi\|_{csy}}
\]
is called the \textbf{coboundary expansion}\index{Coboundary expansion} of \(\varphi\) and
\[
h_k(X)\coloneqq \min_{\substack{\varphi\in C^k(X)\\\varphi\notin\im(\delta^{k-1})}}\|\varphi\|_{exp}
\]
is called the \(k\)-th \textbf{Cheeger constant}\index{Cheeger constant} of \(X\).\\
\\
A cosystole \(\varphi\in C^k(X)\setminus\im(\delta^{k-1})\) is called a \(k\)-\textbf{Cheeger cosystole}\index{Cheeger cosystole} if \(h_k(X)=\|\varphi\|_{exp}\).\\
\\
