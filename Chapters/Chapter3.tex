...
%\]Structure theory of Cheeger graphs
...
So, if \(3\nmid n\) then \(i_n(G)\) is not a Cheeger graph, since by \cite{1} (Theorem 4.1) we have:
\[
...
\begin{proof}
Since \(n\) is divisible by 3 we have \(h(G)=\frac{n}{3}\) by \cite{1} (Corollary 4.3) and so we get\\
\(h(i_n(G))=\frac{n}{3}+1\) by Lemma \ref{lemma4}. Now, since \(n+1\) is not a power of 2 we also know that \(h_1(\Delta^{[n+1]})=\frac{n+1}{3}\) by \cite{1} (Corollary 4.3). Hence, we have:
\[
...
\begin{expl}
In \cite{1} (Section 6.3) Kozlov already used the fact, that a vertex in such a graph can not attain an odd degree to prove the equality \(h(8)>\frac{8}{3}\). Applying the preceding slightly stronger proposition we get this result immediately, since then every vertex in a Cheeger graph\\
