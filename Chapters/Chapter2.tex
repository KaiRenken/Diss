...
The preceding lemma means that assuming there exists a cosystole \(\varphi\in C^1(\Delta^{[n]})\), such that we can find a piercing set for \(\mathcal{T}_{\varphi}\) which is smaller than \(\|\varphi\|\), we will not only need to remove simplices from the support of \(\varphi\) but also add simplices to it to construct this piercing set. But the following statement says that we will need to exchange at least three simplices by two in the previously described way.

\begin{lem}
Let \(\varphi\in C^1(\Delta^{[n]})\) be a cosystole and \(\psi\in C^1(\Delta^{[n]})\) such that we have \(\|\psi\|=3\) and \(|\supp(\psi)\cap\supp(\varphi)|=2\). Then \(P\coloneqq\supp(\varphi + \psi)\) is not a piercing set of \(\mathcal{T}_{\varphi}\).
\begin{proof}
Let \(\{sigma_1,\sigma_2\}=\supp(\psi)\cap\supp(\varphi)\). First, consider the case \(\sigma_1\cap\sigma_2=\emptyset\). 
\end{proof}
\end{lem}
...
%\[
%P_w\coloneqq \left\{c\in\mathcal{T}_{\varphi}:w\in c\text{, }w'\notin c\text{ for all }w'\in X^{(k)}\setminus\sigma\text{, }w'\neq w\right\}
%\]
...
%
%\begin{defi}
%Let \(V\) be some set, \(\mathcal{F}\subseteq 2^V\) a family of subsets of \(V\) and \(P=(p_1,\ldots,p_m)\) a finite ordered piercing set of \(\mathcal{F}\). Furthermore, set \(P_0\coloneqq \emptyset\) and for each \(i=1,\ldots,m\) set \(P_i\coloneqq \left\{F\in\mathcal{F}:F\cap\{p_i\}\neq\emptyset\right\}\setminus P_{i-1}\). Then the tuple \(\lambda_P\coloneqq (|P_1|,\ldots,|P_m|)\) is called the \textbf{piercing sequence} of \(P\).
%\end{defi}
%
%\begin{prop}
