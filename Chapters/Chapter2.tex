...
Nevertheless, we conjecture that for the \(1\)-dimensional case equality holds, since removing a simplex from the support of a \(1\)-cosystole \(\varphi\) can never result in a piercing set of \(\mathcal{T}_{\varphi}\) as the following lemma shows.

\begin{lem}\label{lemma211}
Let \(\varphi\in C^1(\Delta^{[n]})\) be a cosystole, \(P\subseteq\binom{[n]}{2}\) and \(\sigma\in\binom{[n]}{2}\), such that \(P=\supp(\varphi)\setminus\sigma\). Then \(P\) is not a piercing set of \(\mathcal{T}_{\varphi}\).
\begin{proof}
...
\[
\|\delta^0(w^*)\|\geq\left\lceil\frac{n-2}{2}\right\rceil+1=\left\lceil\frac{n-2}{2}+1\right\rceil=\left\lceil\frac{n}{2}\right\rceil>\left\lfloor\frac{n-1}{2}\right\rfloor,
\]
so \(\varphi\) is not cosystolic and we are done.
\end{proof}
\end{lem}
